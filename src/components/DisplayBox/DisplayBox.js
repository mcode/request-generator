...
export default class DisplayBox extends Component {
  ...
  renderSource(source) {
    ...
    if (source.icon) {
      icon = (
        <img className={styles["card-icon"]} src={source.icon} alt="Could not fetch icon" width="100" height="100" />
      );
    }
    if (!this.props.isDemoCard) {
      return (
        <div className={styles["card-source"]}>
          Source:{" "}
          <a className={styles["source-link"]} href={source.url || "#"} onClick={(e) => this.launchSource(e)}>
            {source.label}
          </a>
          {icon}
        </div>
  ...
  render() {
    ...
    if (this.state.response != null && this.state.response.cards != null) {
      this.state.response.cards
        ...
        .forEach((c, cardInd) => {
          ...
          // -- Summary --
          const summarySection = <Header color={summaryColors[card.indicator]}>{card.summary}</Header>;
          ...
          if (card.source.hasOwnProperty("topic")) {
            typeSection = card.source.topic.display ? (
              <div style={{ color: summaryColors.info }}>
                <ReactMarkdown source={card.source.topic.display} />
              </div>
            ) : (
              <Input disabled>None</Input>
            );
          ...
          const builtCard = (
            <Card key={cardInd} className="decision-card alert-info">
              ...
              <div className={styles["suggestions-section"]}>{suggestionsSection}</div>
    ...
    if (renderedCards.length === 0) {
      return (
        <div>
          <div className="decision-card alert-warning">No Cards</div>
        </div>
      );
    }
    return (
